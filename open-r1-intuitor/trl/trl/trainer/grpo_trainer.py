    ...
    def _compute_loss(self, model, inputs):
        ...
        if self.loss_type == "spo":
            ...
            per_token_loss = -(
                ...
                torch.abs(advantages.unsqueeze(1)) * torch.pow(ratio - 1, 2) / (2 * self.epsilon_high)
            )
        else:
            # PPO-style clipping
            coef_1 = ratio
        ...
        if self.loss_type == "grpo":
            ...
            # SPO typically uses mean aggregation similar to GRPO
            loss = ((per_token_loss * completion_mask).sum(-1) / completion_mask.sum(-1).clamp(min=1.0)).mean()
        else:
        ...
        # Compute the clipped probability ratios (only for PPO-style losses)
        if self.loss_type != "spo":
            is_low_clipped = (ratio < 1 - self.epsilon_low) & (advantages.unsqueeze(1) < 0)
            is_high_clipped = (ratio > 1 + self.epsilon_high) & (advantages.unsqueeze(1) > 0)
